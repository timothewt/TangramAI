...
class StatsHandler:
    ...
    def save_data(self, solve_duration: float, corners_number: int, used_pieces: list[Piece]) -> None:
        """
    ...
    def save_solution_steps(self, pieces_used: list[Piece], image: np.ndarray, folder_name: str = None) -> None:
        """
        ...
        :param folder_name: name of the folder in which we stor the steps
        """
        if folder_name is None:
            folder_name = self.puzzle_name
        image_rgb = cv.cvtColor(image, cv.COLOR_GRAY2BGR)
        cv.imwrite(folder_name + "step0.png", image_rgb)
        ...
        for i in range(len(pieces_used)):
            ...
            cv.imwrite(folder_name + "step" + str(i + 1) + ".png", image_rgb)
